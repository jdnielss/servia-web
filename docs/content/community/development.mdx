...
This guide explains how to run Casa Familia Padel Club without Docker. They cover database setup, configuration and
...
To run Casa Familia Padel Club (frontend and backend) locally without Docker, one needs `pnpm` and `uv`.

The following starts the frontend and backend for local development in the root
directory of Casa Familia Padel Club:
...
### Alternative: running Casa Familia Padel Club via process-compose
