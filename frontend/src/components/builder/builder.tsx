...
import { IconDots, IconPencil, IconTrash, IconUsersPlus } from '@tabler/icons-react';
...
function StageItemRow({
  ...
  async function insertFromTeams() {
    const emptyInputs = stageItem.inputs
      .filter((input) => input.team_id == null && input.winner_from_stage_item_id == null)
      .sort((i1, i2) => (i1.slot > i2.slot ? 1 : -1));
    const availableTeamOptions = availableInputs.filter(
      (option) => option.team_id != null && !option.already_taken
    );

    const updates = emptyInputs.slice(0, availableTeamOptions.length);
    for (const [index, input] of updates.entries()) {
      const option = availableTeamOptions[index];
      await updateStageItemInput(
        tournament.id,
        stageItem.id,
        input.id,
        option.team_id || null,
          ...
          <Group gap="0rem">
            ...
            <Menu withinPortal position="bottom-end" shadow="sm">
              ...
              <Menu.Dropdown>
                ...
                <Menu.Item
                  leftSection={<IconUsersPlus size="1.5rem" />}
                  onClick={insertFromTeams}
                >
                  Insert from teams
                </Menu.Item>
                <Menu.Item
