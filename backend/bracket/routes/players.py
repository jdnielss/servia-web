...
from bracket.models.db.player import Player, PlayerBody, PlayerMultiBody, PlayerReclubBody
...
def _extract_confirmed_participant_names(page: str) -> list[str]:
    ...
    return [html.unescape(name).strip() for name in participants if name.strip()]
...
@router.post("/tournaments/{tournament_id}/players_reclub", response_model=SuccessResponse)
async def create_players_from_reclub(
    player_body: PlayerReclubBody,
    tournament_id: TournamentId,
    user: UserPublic = Depends(user_authenticated_for_tournament),
    _: Tournament = Depends(disallow_archived_tournament),
) -> SuccessResponse:
    page = await _fetch_reclub_page(player_body.reclub_link)
    player_names = _extract_confirmed_participant_names(page)
    existing_players = await get_all_players_in_tournament(tournament_id)
    check_requirement(existing_players, user, "max_players", additions=len(player_names))
