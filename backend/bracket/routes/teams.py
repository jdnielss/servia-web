...
async def _fetch_reclub_page(reclub_link: str) -> str:
    async with aiohttp.ClientSession() as session:
        async with session.get(
            reclub_link, headers={"User-Agent": "Mozilla/5.0"}, timeout=20
        ) as response:
            response.raise_for_status()
            return await response.text()


def _extract_confirmed_players_as_pairs(page: str) -> list[tuple[str, str]]:
    start = page.find("Dikonfirmasi <span>·")
    if start == -1:
        raise ValueError("Could not find confirmed participants section")
    ...
    if len(names) < 2:
        raise ValueError("No participants found in confirmed section")

    # Keep only complete pairs from sequential confirmed participants.
    if len(names) % 2 != 0:
        names = names[:-1]
...
@router.post("/tournaments/{tournament_id}/teams_reclub", response_model=SuccessResponse)
async def create_teams_from_reclub(
    team_body: TeamReclubBody,
    tournament_id: TournamentId,
    user: UserPublic = Depends(user_authenticated_for_tournament),
    _: Tournament = Depends(disallow_archived_tournament),
) -> SuccessResponse:
    page = await _fetch_reclub_page(team_body.reclub_link)
    pairs = _extract_confirmed_players_as_pairs(page)

    existing_teams = await get_teams_with_members(tournament_id)
    existing_players = await get_all_players_in_tournament(tournament_id)

    check_requirement(existing_teams, user, "max_teams", additions=len(pairs))
    check_requirement(existing_players, user, "max_players", additions=len(pairs) * 2)
    ...
    async with database.transaction():
        for player1, player2 in pairs:
            ...
            for player_name in (player1, player2):
                player_body = PlayerBody(name=player_name, active=team_body.active)
                await insert_player(player_body, tournament_id)
